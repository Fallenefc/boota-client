...
import {
  apiGetFullExamAsAStudent,
  submitAnExamAsAStudent,
...
export default function StudentExam(): ReactElement {
  ...
  const handleExamSubmission = async () => {
    const response: any = await submitAnExamAsAStudent(
      exam.hashedId,
      answers,
      questionsIds,
    );
  ...
  return (
    <div className="student-exam-container">
      ...
      {exam ? (
        exam.options.map((option: any, index: number) => (
          <StudentQuestionCard
            stem={option.stem}
      ...
      <button className="submit-exam-button" onClick={handleExamSubmission} type="button">
        Submit Your Exam
      </button>
    </div>
  );
